...
class ChatifyMessenger
{
    ...
    public function __construct()
    {
        ...
        config([
            'chatify.pusher.key' => isset($setting['pusher_app_key']) ? $setting['pusher_app_key'] : '',
            'chatify.pusher.secret' => isset($setting['pusher_app_secret']) ? $setting['pusher_app_secret'] : '',
            'chatify.pusher.app_id' => isset($setting['pusher_app_id']) ? $setting['pusher_app_id'] : '',
            'chatify.pusher.options.cluster' => isset($setting['pusher_app_cluster']) ? $setting['pusher_app_cluster'] : '',
        ]);
    ...
    public function getContactItem($messenger_id, $user){
        // get last message
        $lastMessage = $this->getLastMessageQuery(!empty($user)?$user->id:0);

        // Get Unseen messages counter
        $unseenCounter = $this->countUnseenMessages(!empty($user)?$user->id:0);
