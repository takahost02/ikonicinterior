                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($job->status=='pending')
                                            <span class="badge status_badge bg-warning p-2 px-3 rounded">{{\App\Models\JobOnBoard::$status[$job->status]}}</span>
                                        @elseif($job->status=='cancel')
                                            <span class="badge status_badge bg-danger p-2 px-3 rounded">{{\App\Models\JobOnBoard::$status[$job->status]}}</span>
                                        @else
                                            <span class="badge status_badge bg-primary p-2 px-3 rounded">{{\App\Models\JobOnBoard::$status[$job->status]}}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <div class="action-btn me-2">
                                            ...
                                            {!! Form::open(['method' => 'get', 'route' => ['job.on.board.converts', $job->id],'id'=>'job-form-'.$job->id]) !!}
                                            ...
                                            <div class="action-btn me-2">
                                                <a href="{{route('employee.show', \Crypt::encrypt($job->convert_to_employee))}}" class="mx-3 btn btn-sm align-items-center bg-warning" data-bs-toggle="tooltip" title="{{__('View')}}" data-original-title="{{__('Employee Detail')}}"><i class="ti ti-eye text-white"></i></a>
                                            </div>
                                            @endif

                                            <div class="action-btn me-2">
                                                <a href="#" data-url="{{route('job.on.board.edit', $job->id)}}" data-ajax-popup="true" class="mx-3 btn btn-sm align-items-center bg-info" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-original-title="{{__('Edit')}}"><i class="ti ti-pencil text-white"></i></a>
                                            </div>
