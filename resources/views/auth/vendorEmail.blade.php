        ...
        <li class="dropdown dash-h-item drp-language">
            ...
            <div class="dropdown-menu dash-h-dropdown dropdown-menu-end">
                @foreach($languages as $code => $language)
                    <a href="{{ route('vender.change.langPass',$code) }}" tabindex="0" class="dropdown-item {{ $code == $lang ? 'active':'' }}">
...
@section('content')
    ...
    <form method="POST" action="{{ route('vender.password.email') }}" class="needs-validation" novalidate>
        ...
        <div class="">
            ...
            <p class="my-4 text-center">{{__('Back to')}} <a href="{{ route('vender.login') }}" class="text-primary">{{__('Login')}}</a></p>
