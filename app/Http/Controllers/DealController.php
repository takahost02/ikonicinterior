...
//                ClientPermission::where('deal_id', '=', $deal->id)->delete();
    ...
    public function order(Request $request)
    {
        ...
        if($usr->can('move deal'))
        {
            ...
            return response()->json(['status' => 'success','message' => __('Deal successfully moved.')], 200);
        }
        else
        {
        ...
        if ($request->hasFile('file') && $request->file->getClientOriginalName() != '') {
            ...
            if ($extension == 'csv') {
                ...
                while (($row = fgetcsv($file_data)) !== false) {
                    ...
                    $clients = User::where('type', 'client')->where('created_by', \Auth::user()->creatorId())->pluck('name', 'id');
    ...
    {
        return view('deals.import_modal');
    }
    ...
    public function dealImportdata(Request $request)
    {
        ...
        if ($user->default_pipeline) {
            $pipeline = Pipeline::where('created_by', '=', \Auth::user()->creatorId())->where('id', '=', $user->default_pipeline)->first();
            if (!$pipeline) {
        ...
        foreach ($file_data as $key => $row) {
            try {
                ...
                if (empty($client)) {
                    $client = User::where('type','client')->where('created_by', \Auth::user()->creatorId())->first();
                }
                $deal = Deal::create([
                ...
                $html .= '<td>' . (isset($row[$request->price]) ? $row[$request->price] : '-' ) . '</td>';
